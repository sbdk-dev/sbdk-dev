...
from sbdk.cli.commands.run import load_config, run_pipeline_module
    ...
    def test_dev_command_functionality(self, mock_run):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.exit_code in [
                0,
                1,
            ]:  # May fail due to missing files, but shouldn't crash
                pass
            else:
...
class TestWebhooksCommandComprehensive:
    ...
    def test_webhooks_with_custom_port(self, mock_run):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            runner = CliRunner()
            runner.invoke(app, ["webhooks", "--port", "9000"])
...
class TestCoreConfigComprehensive:
    ...
    def test_config_class_creation(self):
        ...
        if (
            SBDKConfig is not None
            and hasattr(SBDKConfig, "__init__")
            and callable(SBDKConfig)
        ):
            # SBDKConfig requires project and duckdb_path parameters
            config = SBDKConfig(project="test_project", duckdb_path="test.db")
    ...
    def test_config_validation(self):
        ...
        if hasattr(SBDKConfig, "validate"):
            # SBDKConfig requires project and duckdb_path parameters
            config = SBDKConfig(project="test_project", duckdb_path="test.db")
...
class TestCoreProjectComprehensive:
    ...
    def test_project_methods_exist(self):
        ...
        expected_methods = ["create", "validate", "get_config"]
