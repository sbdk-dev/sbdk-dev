...
class TestCLIPerformance:
    ...
    def test_init_command_performance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"Init command took {metrics.execution_time:.3f}s, used {metrics.memory_usage:.1f}MB")
    ...
    def test_config_loading_performance(self):
        ...
        test_cases = [
            ...
            ("large", {"project": "test", "data": list(range(10000)), "config": {f"key_{i}": f"value_{i}" for i in range(1000)}})
        ]
...
class TestDataGenerationPerformance:
    ...
    def test_users_generation_scalability(self):
        ...
        for scale in scales:
            ...
            print(f"Generated {scale} users in {metrics.execution_time:.3f}s ({scale/metrics.execution_time:.0f} users/sec)")
    ...
    def test_events_generation_scalability(self):
        ...
        for scale in scales:
            ...
            print(f"Generated {scale} events in {metrics.execution_time:.3f}s ({scale/metrics.execution_time:.0f} events/sec)")
    ...
    def test_orders_generation_scalability(self):
        ...
        for scale in scales:
            ...
            print(f"Generated {scale} orders in {metrics.execution_time:.3f}s ({scale/metrics.execution_time:.0f} orders/sec)")
...
class TestDatabasePerformance:
    ...
    def test_duckdb_creation_performance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"Inserted 10k records in {metrics.execution_time:.3f}s ({10000/metrics.execution_time:.0f} records/sec)")
    ...
    def test_duckdb_query_performance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            queries = [
                ...
                ("JOIN query", "SELECT COUNT(*) FROM users u JOIN events e ON u.user_id = e.user_id"),
                ("Aggregation", "SELECT subscription_tier, COUNT(*) FROM users GROUP BY subscription_tier"),
                ("Complex query", """
        ...
        try:
            ...
            for i in range(5):
                thread = threading.Thread(
                    ...
                    args=([f"init", f"concurrent_project_{i}"], temp_dirs[i])
                )
            ...
            assert successful == 5
            print(f"5 concurrent CLI instances took {metrics.execution_time:.3f}s total")
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_large_datasets(self):
        ...
        print(f"Memory usage - Users: {users_memory:.1f}MB, Events: {events_memory:.1f}MB, Total: {total_memory:.1f}MB")
    ...
    def test_maximum_dataset_size(self):
        ...
        for dataset_type, max_size in max_sizes.items():
            ...
            try:
                ...
                assert metrics.execution_time < 120  # Allow more time for large datasets in CI
                
                print(f"Generated {max_size} {dataset_type} in {metrics.execution_time:.1f}s")
    ...
    def test_database_size_limits(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for batch in range(max_batches):
                ...
                print(f"Batch {batch + 1}: {total_records} records, DB size: {db_size:.1f}MB")
