...
from sbdk.cli.integration_layer import SBDKEnhancedCLI, SBDKVisualIntegration
...
class TestFastAPIClient:
    ...
    async def test_health_check_failure(self, client):
        ...
        with patch.object(
            client.client, "get", side_effect=Exception("Connection failed")
        ):
            result = await client.health_check()
    ...
    async def test_track_usage_with_registration(self, client):
        ...
        mock_response = Mock()
        mock_response.json.return_value = {"status": "tracked", "event_id": "event-123"}
        mock_response.raise_for_status = Mock()
...
class TestStatusPanel:
    ...
    def test_add_status_item(self):
        ...
        panel.add_status_item(
            "server", "API Server", "Running", ComponentState.SUCCESS, "🟢"
        )
    ...
    def test_update_status_item(self):
        ...
        panel = StatusPanel()
        panel.add_status_item(
            "server", "API Server", "Starting", ComponentState.LOADING
        )
...
class TestEndToEndIntegration:
    ...
    def test_component_integration(self, complete_project):
        ...
        assert (
            cli.integration.status_panel.status_items.get("test", {}).get("value")
            == "Active"
        )
