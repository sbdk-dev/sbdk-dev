...
class TestRealTimeInteraction:
    ...
    def test_file_watcher_simulation(self):
        ...
        handler = PipelineFileHandler(
            {"project": "test", "duckdb_path": "test.db"}, visual=False
        ...
        with patch("sbdk.cli.commands.run.execute_pipeline"):
...
class TestUserExperienceFlows:
    ...
    def test_helpful_next_steps(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output = result.stdout
            next_step_indicators = ["Next steps", "cd ", "python", "pip install", "dev"]
    ...
    def test_error_message_quality(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            helpful_elements = ["not found", "init", "first", "sbdk"]

            has_helpful_error = any(
                element in error_output for element in helpful_elements
            )
    ...
    def test_production_deployment_prep(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open("sbdk_config.json") as f:
                ...
                assert all(
                    key in config
                    for key in ["project", "duckdb_path", "dbt_path", "profiles_dir"]
                )
