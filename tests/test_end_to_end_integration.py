            ...
            try:
                ...
                with patch("subprocess.run") as mock_run:
                    mock_run.return_value = MagicMock(
                        returncode=0, stdout="Pipeline completed"
                    )
                ...
                with patch("subprocess.run") as mock_dbt:
                    mock_dbt.return_value = MagicMock(
                        returncode=0, stdout="dbt completed"
                    )
    ...
    def test_data_quality_validation(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            null_ids = con.execute(
                "SELECT COUNT(*) FROM users_quality_test WHERE user_id IS NULL"
            ).fetchone()[0]
            assert null_ids == 1

            # Check for invalid emails
            invalid_emails = con.execute(
                "SELECT COUNT(*) FROM users_quality_test WHERE email NOT LIKE '%@%'"
            ).fetchone()[0]
            assert invalid_emails == 1

            # Check for null dates
            null_dates = con.execute(
                "SELECT COUNT(*) FROM users_quality_test WHERE created_at IS NULL"
            ).fetchone()[0]
            assert null_dates == 1
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert (
                    ...
                    or "Permission" in str(result.exception)
    ...
    def test_multiple_cli_instances(self):
        ...
        for i in range(3):
            thread = threading.Thread(
                target=run_cli_init, args=[f"concurrent_project_{i}"]
            )
