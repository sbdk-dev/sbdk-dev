...
class TestDBTIntegration:
    ...
    def test_dbt_models_structure(self):
        ...
        models_path = (
            Path(__file__).parent.parent / "sbdk" / "templates" / "dbt" / "models"
        )
    ...
    def test_dbt_model_sql_syntax(self):
        ...
        for sql_file in sql_files:
            ...
            assert (
                "select" in content.lower() or "with" in content.lower()
...
class TestDBTExecution:
    ...
    def test_dbt_run_command(self, mock_run):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            run_called = any(
                "dbt" in str(call) and "run" in str(call) for call in calls
            )
            test_called = any(
                "dbt" in str(call) and "test" in str(call) for call in calls
            )
...
class TestDataTransformations:
    ...
    def test_staging_model_logic(self):
        ...
        assert (
            len(staging_files) >= 3
        ), "Should have staging models for users, events, orders"
...
class TestDBTConfiguration:
    ...
    def test_dbt_dependencies_installed(self):
        ...
        try:
            ...
            pytest.skip(
                "dbt-duckdb not installed in test environment - this is expected"
            )
    ...
    def test_init_to_dbt_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with (
                    patch("sbdk.cli.commands.dev.run_pipeline_module"),
