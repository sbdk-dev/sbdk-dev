...
from sbdk.cli.commands.run import (
    PipelineHandler,
    load_config,
    run_pipeline_module,
)
...
class TestCLIInit:
    ...
    def test_init_basic_project(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                "Successfully initialized SBDK project: test_project" in result.stdout
            )
...
class TestCLIIntegration:
    ...
    def test_version_command(self):
        ...
        assert result.exit_code == 0
        assert "SBDK.dev v1.0.1" in result.stdout
    ...
    def test_path_traversal_prevention(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.exit_code != 0:
                ...
                assert (
                    "Permission denied" in str(result.exception)
                    or result.exit_code == 1
                )

            # Most importantly, check that it doesn't write outside the intended area
            assert not (
                Path(temp_dir).parent.parent.parent / "malicious_project"
            ).exists()
