...
- **Command Structure**:
  - `sbdk init <project>`: Initialize new project
  - `sbdk dev`: Run full pipeline (ingest + transform)
  - `sbdk start`: Development mode with file watching
  - `sbdk webhooks`: Start webhook server
- **Rich UI**: Beautiful terminal output with progress bars
- **Configuration**: Dynaconf-based config management
