...
def generate_events_data(num_events: int = 50000, max_user_id: int = 10000) -> list:
    ...
    for i in range(1, num_events + 1):
        ...
        event = {
            ...
            "user_id": user_id,
            "session_id": str(uuid.uuid4()) if random.random() < 0.1 else None,  # 10% have session tracking
            "event_type": event_type,
            "timestamp": event_time,
            "utm_source": random.choice(weighted_utm),
            "utm_medium": fake.random_element(elements=('cpc', 'organic', 'email', 'social', 'referral')),
            ...
            "duration_seconds": random.randint(1, 1800) if event_type == 'page_view' else None,
...
def run():
    ...
    con.execute("CREATE INDEX IF NOT EXISTS idx_events_timestamp ON raw_events(timestamp)")
    ...
    print(f"""📈 Events Pipeline Results:
