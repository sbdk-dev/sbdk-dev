...
class VisualRenderer:
    ...
    def _detect_color_support(self) -> bool:
        ...
        return (
            os.getenv('COLORTERM') in ('truecolor', '24bit') or
            os.getenv('TERM', '').endswith('256color') or
            os.getenv('TERM') == 'xterm-color'
        )
    ...
    def swap_buffers(self):
        ...
        for line_num in self.dirty_regions:
            if line_num < len(self.back_buffer):
                front_line = self.front_buffer[line_num] if line_num < len(self.front_buffer) else ''
