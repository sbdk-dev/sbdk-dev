...
class Header(Component):
    ...
    def __init__(self, title: str = "SBDK Analytics", status: str = "Ready", 
                 show_time: bool = True, **kwargs):
    ...
    def render(self, renderer):
        ...
        if title_len + status_len + time_len <= available_width:
            ...
            title_line = (v_line + title_content + 
                         ' ' * middle_space + status_content + time_content + v_line)
        else:
            # Truncate title if needed
            remaining = available_width - status_len - time_len - 3  # 3 for "..."
            if remaining > 0:
                truncated_title = title_content[:remaining] + "..."
                title_line = (v_line + truncated_title + 
                             status_content + time_content + v_line)
            else:
...
class Footer(Component):
    ...
    def __init__(self, help_text: str = "Press 'q' to quit", 
                 status: str = "", **kwargs):
        super().__init__(**kwargs)
    ...
    def render(self, renderer):
        ...
        if self.status:
            ...
            if len(status_content) <= available:
                ...
                content_line = v_line + content + ' ' * spacing + status_content + v_line
            else:
                content_line = v_line + content + ' ' * (width - 2 - len(content)) + v_line
        else:
...
class ProgressBar(Component):
    ...
    def __init__(self, progress: float = 0.0, width: int = 40, 
                 label: str = "", style: str = "modern", **kwargs):
        super().__init__(**kwargs)
...
class Spinner(Component):
    ...
    def __init__(self, style: str = "dots", label: str = "Loading", **kwargs):
        ...
        if not renderer.supports_feature('unicode') if hasattr(self, 'renderer') else False:
            self.style = "classic"
...
class Box(Component):
    ...
    def __init__(self, border: bool = True, title: str = "", 
                 border_style: str = "single", padding: int = 1, **kwargs):
        super().__init__(**kwargs)
        self.border = border
        self.title = title
