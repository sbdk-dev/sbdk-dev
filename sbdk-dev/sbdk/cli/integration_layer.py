...
class SBDKVisualIntegration:
    ...
    def load_sbdk_project(self):
        ...
        try:
            ...
            self.log_panel.add_log("SBDK project configuration loaded successfully", "INFO", "Integration")
            
        except Exception as e:
            self.log_panel.add_log(f"Could not load SBDK project: {e}", "WARNING", "Integration")
            ...
            if config_path.exists():
                try:
                    ...
                    self.log_panel.add_log("Loaded fallback configuration", "INFO", "Integration")
                except Exception as config_error:
                    self.log_panel.add_log(f"Config error: {config_error}", "ERROR", "Integration")
                    self.config = self.get_default_config()
            else:
                self.config = self.get_default_config()
                self.log_panel.add_log("Using default configuration", "WARNING", "Integration")
    ...
    def setup_action_callbacks(self):
        ...
        def show_detailed_status():
            ...
            # Project info
            self.log_panel.add_log(f"Project: {self.config.get('project', 'Unknown')}", "INFO", "Status")
            ...
            if db_path.exists():
                ...
                self.log_panel.add_log(f"Database: {db_path} ({size_mb:.1f} MB)", "INFO", "Status")
            else:
                self.log_panel.add_log(f"Database: {db_path} (not found)", "WARNING", "Status")
                
            # Directories
            pipelines_dir = self.project_path / self.config.get("pipelines_dir", "pipelines")
            dbt_dir = self.project_path / self.config.get("dbt_dir", "dbt")
            
            self.log_panel.add_log(f"Pipelines: {pipelines_dir} ({'exists' if pipelines_dir.exists() else 'missing'})", "INFO", "Status")
            self.log_panel.add_log(f"dbt: {dbt_dir} ({'exists' if dbt_dir.exists() else 'missing'})", "INFO", "Status")
        ...
        def open_config_editor():
            ...
            try:
                if os.name == 'nt':  # Windows
                    ...
                    subprocess.run(['open', str(config_path)] if sys.platform == 'darwin' 
                                 else ['xdg-open', str(config_path)])
            except Exception as e:
                self.log_panel.add_log(f"Could not open editor: {e}", "ERROR", "Actions")
                
        def show_help():
            """Show help information"""
            self.log_panel.add_log("=== SBDK.dev VISUAL CLI HELP ===", "INFO", "Help")
            self.log_panel.add_log("Visual CLI v2.0.0 - Production ready terminal interface", "INFO", "Help")
        ...
        self.actions_panel.actions[3]["callback"] = lambda: None        # l (handled by main CLI)
    ...
    def _run_pipeline_async(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_panel.add_log("Users pipeline completed successfully", "INFO", "Pipeline")
            else:
                self.pipeline_panel.complete_step(0, False, "Users pipeline failed")
                self.log_panel.add_log(f"Users pipeline failed: {result.stderr}", "ERROR", "Pipeline")
                return
            ...
            if result.returncode == 0:
                ...
                self.log_panel.add_log("Events pipeline completed successfully", "INFO", "Pipeline")
            else:
                self.pipeline_panel.complete_step(1, False, "Events pipeline failed")
                self.log_panel.add_log(f"Events pipeline failed: {result.stderr}", "ERROR", "Pipeline")
                return
            ...
            if result.returncode == 0:
                ...
                self.log_panel.add_log("Orders pipeline completed successfully", "INFO", "Pipeline")
            else:
                self.pipeline_panel.complete_step(2, False, "Orders pipeline failed")
                self.log_panel.add_log(f"Orders pipeline failed: {result.stderr}", "ERROR", "Pipeline")
                return
            ...
            if result.returncode == 0:
                ...
                self.log_panel.add_log("dbt run completed successfully", "INFO", "Pipeline")
            else:
                self.pipeline_panel.complete_step(3, False, "dbt run failed")
                self.log_panel.add_log(f"dbt run failed: {result.stderr}", "ERROR", "Pipeline")
                return
            ...
            if result.returncode == 0:
                ...
                self.log_panel.add_log("dbt test completed successfully", "INFO", "Pipeline")
            else:
                self.pipeline_panel.complete_step(4, False, "dbt test failed")
                self.log_panel.add_log(f"dbt test failed: {result.stderr}", "ERROR", "Pipeline")
                return
                
            # Success!
            duration = datetime.now() - start_time
            self.status_panel.update_status_item("pipeline", f"Completed ({duration.seconds}s ago)")
            self.log_panel.add_log(f"✅ Full pipeline completed in {duration.seconds}s", "INFO", "Pipeline")
            
        except Exception as e:
            self.log_panel.add_log(f"Pipeline execution error: {e}", "ERROR", "Pipeline")
    ...
    def update_metrics(self):
        ...
        try:
            ...
            self.metrics_panel.update_metric("records_per_sec", random.randint(100, 500) if self.pipeline_running else 0)
...
class SBDKEnhancedCLI(VisualCLI):
    ...
    async def initialize(self):
        ...
        # Initial log
        self.integration.log_panel.add_log("SBDK Visual CLI v2.0.0 initialized", "INFO", "System")
    ...
    def render_enhanced_dashboard(self, layout):
        ...
        layout["header"].update(Panel(
            f"🚀 SBDK.dev Visual CLI v2.0.0 - {project_name}",
            style="bold blue"
        ))
