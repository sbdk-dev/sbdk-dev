...
def cli_debug(
    ...
    dbt_only: bool = typer.Option(False, "--dbt-only", help="Debug only dbt configuration"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show verbose output")
):
    """Debug SBDK configuration and environment setup"""
    
    console.print(Panel(
        "[cyan]🔍 SBDK Debug Information[/cyan]",
        title="Debug Mode",
        style="cyan"
    ))
    ...
    if not dbt_only:
        ...
        sys_table.add_row("Virtual Environment", os.environ.get('VIRTUAL_ENV', 'Not active'))
        sys_table.add_row("PATH", os.environ.get('PATH', '')[:100] + "..." if len(os.environ.get('PATH', '')) > 100 else os.environ.get('PATH', ''))
            ...
            for key, value in config.items():
                ...
                if key in ["dbt_path", "profiles_dir", "duckdb_path"]:
                    ...
                    if key == "duckdb_path":
                        ...
                        status = "✅" if path.parent.exists() else "❌ Parent dir missing"
                    else:
                        status = "✅" if path.exists() else "❌ Not found"
                
                config_table.add_row(key, str(value)[:80] + "..." if len(str(value)) > 80 else str(value), status)
    ...
    try:
        ...
        try:
            ...
            console.print(f"[green]✅ dbt runner initialized successfully[/green]")
            ...
            if debug_info['exit_code'] == 0:
                ...
                console.print(f"[red]❌ dbt debug failed (exit code: {debug_info['exit_code']})[/red]")
        ...
        console.print("1. Install dbt: pip install dbt-duckdb")
        console.print("2. Or with uv: uv pip install dbt-duckdb")
    ...
    if not dbt_only:
        ...
        try:
            ...
            paths_to_check = [
                ...
                ("data directory", Path(config.get("duckdb_path", "")).parent if config.get("duckdb_path") else None)
            ]
            ...
            for name, path in paths_to_check:
                ...
                fs_table.add_row(name, str(path)[:60] + "..." if len(str(path)) > 60 else str(path), status)
    ...
    if not dbt_only:
        ...
        console.print("[dim]- Run 'sbdk dev --pipelines-only' to test pipeline execution[/dim]")
