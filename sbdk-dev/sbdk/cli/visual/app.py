...
from .components import Component, Header, Footer
...
class VisualApp:
    ...
    def set_header(self, title: str = None, status: str = "Ready", 
                   show_time: bool = True, **kwargs) -> Header:
    ...
    def set_footer(self, help_text: str = "Press 'q' to quit", 
                   status: str = "", **kwargs) -> Footer:
...
def create_simple_app(title: str = "SBDK CLI", show_header: bool = True, 
                     show_footer: bool = True) -> VisualApp:
...
def quick_demo():
    ...
    from .components import ProgressBar, Spinner, Box
    import time
    ...
    progress = ProgressBar(
        x=5, y=layout['content']['y'] + 2,
        width=40, label="Processing"
    )
    
    spinner = Spinner(
        x=5, y=layout['content']['y'] + 4,
        style="dots", label="Loading modules"
    )
    
    info_box = Box(
        x=5, y=layout['content']['y'] + 6,
        width=50, height=8,
        title="System Status",
        border=True
    )
    info_box.set_content([
        ...
        "✓ Unicode Support: " + ("Yes" if app.renderer.supports_feature('unicode') else "No"),
        "✓ Color Support: " + ("Yes" if app.renderer.supports_feature('color') else "No"),
        "",
