...
class StatusPanel(BaseComponent):
    ...
    def add_status_item(self, key: str, label: str, value: str, 
    ...
    def update_status_item(self, key: str, value: str, 
...
class PipelineProgressPanel(BaseComponent):
    ...
    def render(self) -> Panel:
        ...
        content.append(f"Progress: [{progress_bar}] {self.overall_progress:.1%}\n\n", style="bold")
...
class LogPanel(BaseComponent):
    ...
    def add_log(self, message: str, level: str = "INFO", 
                component: str = "", timestamp: Optional[datetime] = None):
        """Add a log entry"""
        entry = {
    ...
    def set_filter(self, level: Optional[str] = None, search_term: Optional[str] = None):
        """Set log filtering"""
...
class MetricsPanel(BaseComponent):
    ...
    def add_metric(self, key: str, name: str, unit: str = "", 
    ...
    def render(self) -> Panel:
        ...
        for key, metric in self.metrics.items():
            ...
            min_val = metric["format_fn"](metric["min_value"]) if metric["min_value"] != float('inf') else "N/A"
            max_val = metric["format_fn"](metric["max_value"]) if metric["max_value"] != float('-inf') else "N/A"
            avg_val = metric["format_fn"](metric["avg_value"]) if metric["values"] else "N/A"
...
class ActionPanel(BaseComponent):
    ...
    def add_action(self, key: str, label: str, description: str = "",
...
def create_server_status_panel() -> StatusPanel:
    ...
    panel.add_status_item("fastapi", "FastAPI Server", "Unknown", ComponentState.IDLE, "🌐")
    panel.add_status_item("database", "DuckDB", "Unknown", ComponentState.IDLE, "🦆") 
    panel.add_status_item("filewatch", "File Watcher", "Unknown", ComponentState.IDLE, "👁️")
    panel.add_status_item("pipeline", "Last Pipeline", "Not run", ComponentState.IDLE, "🔄")
    return panel
...
def create_quick_actions() -> ActionPanel:
    ...
    panel.add_action("a", "Toggle Auto-run", "Enable/disable automatic pipeline execution")
