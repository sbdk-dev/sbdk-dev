    ...
    try:
        ...
        if not Path(config_path).exists():
            ...
            for path in potential_paths:
                ...
                console.print(f"[dim]Searched paths: {[str(p) for p in potential_paths]}[/dim]")
                raise typer.Exit(1)
        ...
        if "profiles_dir" in config:
            config["profiles_dir"] = str(Path(config["profiles_dir"]).expanduser().resolve())
    ...
    with Progress(
        ...
        if not pipelines_only:
            ...
            try:
                ...
                runner.run(debug=False)  # Disable debug output for cleaner progress display
                progress.advance(dbt_task)
                
                # dbt test
                progress.update(dbt_task, description="Running dbt tests...")
                runner.test(debug=False)  # Disable debug output for cleaner progress display
                progress.advance(dbt_task)
                ...
                try:
                    ...
                    if debug_info:
                        console.print(f"[dim]dbt executable: {debug_info['dbt_path']}[/dim]")
                        console.print(f"[dim]Project directory: {debug_info['project_dir']}[/dim]")
                        console.print(f"[dim]Profiles directory: {debug_info['profiles_dir']}[/dim]")
                except:
                    console.print("[yellow]Could not retrieve debug information[/yellow]")
